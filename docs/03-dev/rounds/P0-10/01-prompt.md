...
Source: User（批次指令文件 `docs/03-dev/rounds/cmd_p0-10.md`，属批次 `cmd_batch_p0-10_to_p0-13.md`）

> ⚠️ 本文件是**原始指令档案**。正文按协议 §六 保持原样：不总结、不改写、不美化、不删边界条件。
> 批次级约束（自动继续条件 / 停止条件 / 累积工作区 / 架构硬约束 / Git 禁令）见同目录
> `docs/03-dev/rounds/cmd_batch_p0-10_to_p0-13.md`，本文件不复制。
