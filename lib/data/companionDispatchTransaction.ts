 ...
 * 5. **再看这一单是不是他自己下的**——一个人不能接自己下的单（EX-DISPATCH-08「禁止自接单」），
 *    公共池与专属池都适用。⚠️ 它**不**并进第 3 步：第 3 步只在专属池成立，
 *    而这一条与池子无关。它排在最后、紧邻写入区段，是因为它是唯一一条
 *    「这两个人是不是同一个人」的判定——前面的每一步回答的都是
 *    「这张单还能不能被接」或「这个人还能不能接单」。
 *
...
export async function acceptDispatch(
  ...
  // 一个人不能接自己下的单（EX-DISPATCH-08「禁止自接单」）。公共池与专属池**都适用**，
  // 因此这条判定放在池子相关的判定（第 3 步）之外，对两条路径同时成立。
  //
  // ⚠️ 它**必须在服务端**、且**不能依赖池子 DTO**：公共池 DTO 刻意不含 `userId`
  // （BF-16「接单前只给接单决策所需最小信息」、权限表 §10 数据最小化），
  // 打手在池子里本来就分不出哪张单是自己的——「前端不显示那个按钮」在这里连
  // 一个可用的判据都没有。页面上那一步隐藏按钮做不到这件事。
  //
